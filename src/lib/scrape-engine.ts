...
function extractAddressFromMapsText(text: string, title: string): string {
  ...
  const patterns = [
    /\d{1,6}\s+[A-Za-z0-9'â€™.\-\/& ]{2,80}\s+(?:St|Street|Ave|Avenue|Rd|Road|Blvd|Boulevard|Dr|Drive|Way|Ln|Lane|Ct|Court|Cres|Crescent|Pkwy|Parkway|Pl|Place|Ter|Terrace|Hwy|Highway)\b[^|]{0,80}/i,
    /\d{1,6}\s+[A-Za-z0-9'â€™.\-\/& ]{2,80}\s+(?:Kitchener|Waterloo|Guelph|Hamilton|Cambridge|Toronto|London|Burlington|Ontario|ON)\b[^|]{0,80}/i,
  ];
